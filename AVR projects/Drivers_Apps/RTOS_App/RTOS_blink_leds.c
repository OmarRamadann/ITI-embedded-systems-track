...
//	DIO_ErrStateSetPinValue(DIO_GroupD, Pin7, DIO_High);
	Set_Bit(portd,7); //led 1 stays on each 1 sec
//	_delay_ms(1000);
//	Tog_Bit(portd,7);
	counter1=0;
	}
...
void led2(void)
{
	static u16 counter2;
		...
		{
	Tog_Bit(portd,6);
	_delay_ms(1000); //led 2 turns on after 2 sec then turns off
	Tog_Bit(portd,6);
...
void led3(void)
{
	static u16 counter3;
			...
			{
	Tog_Bit(portd,5);
	_delay_ms(1000); //led 3 turns on after 3 sec then turns off
	Tog_Bit(portd,5);
	counter3=0;
			}
}
