...
Res[1]=DIO_ErrStateSetGroupDirection(DIO_GroupD, 0x00);
...
while(1)
{
	if (Res[0]==NoError &&Res[1]==NoError)
	{
		...
		Res[3]=DIO_ErrStateReadPinValue(DIO_GroupD, Pin1, &Switch2);
		...
		if (Res[2]==NoError && Switch1==pressed)
		{
			...
			for (counter=0;counter<SevSeg_Arrize;counter++)
					{
					Res[4]=DIO_ErrStateReadPinValue(DIO_GroupD, Pin2, &Switch3);//needs interrupt
		 ...
		 if( Res[3]==NoError &&Switch2==pressed)
					{

						for (counter=SevSeg_Arrize-1;counter>=0;counter--)
								{
